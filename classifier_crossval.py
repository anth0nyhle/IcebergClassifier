...
if __name__ == "__main__":
    ...
    for train, test in kfold.split(images, labels):
        ...
        csv_logger = callbacks.CSVLogger("test8_9lay_10crossval_epoch_results.log", separator=",", append=True)
    ...
    np.savetxt("test8_9lay_10crossval_predlabels.csv", labels_test, delimiter=",")
    np.savetxt("test8_9lay_10crossval_dev_acc.csv", cvscores, delimiter=",")

    # with open("ids.csv", "w") as idsfile:
    #     wr = csv.writer(idsfile, dialect="excel")
    #     wr.writerow(ids)

    with open("test8_submission.csv", "w") as submission_file:
